...
This document is a brief description of over 20 years research work. We are not mathematicians. We only want to solve the common problems of the computing world from the practitioner's perspective. We are aware of the havoc quantum computing will bring to the modern cryptography. Consequently, we have prepared ourselves a decade in advance. We have used AES to develop a 2048-Bit AES-hybrid. This is a low-cost, high entropy, end-point to end-point cryptographic system for cloud, mobile and IoT devices (ECSMID). This reference implementation specifies a novel category of hard NP problem closely related to numbered elements in the face of a lattice basis or matrix. This cryptographic proposal shows the properties of both symmetric, asymmetric cryptography or public key infrastructure (PKE, KEM and DS).
*******************************************************************************************************************************!
*******************************************************************************************************************************!
# Abstract

Many works on lattice basis use one reduction in bias especially deterministic one to prove that the lattices are of an interesting hard problem within the set of NP Complete problems. The shortest vector problem (SVP) is not enough to facilitate and establish the lattice basis an exception from the priori art. The many configuration of the vertices dismisses the wonderful properties of their dynamic faces. The elements of these faces found in between regions bounded by the vertices are of great interest for cryptography. When represented as numerical values serve as Mathematical images of the basis distribution. In this paper, it is shown that each vector has the potential to generate cryptographically secure number of keys. This follows a rigid rule; deterministic and yet a chaotic arrangement of the lattice vectors represented within a matrix of column  (c) and rows (r), where (c=>16 and r=>16). A fitting rule, is that, found in Open Knight Tour (OKT) solutions. They come ready made with properties that are closely related to the regular vectors of Euclidean space. 

# OBJECTIVE:
To show that there is a cryptographic approach that fits into the ideal sets of algorithms known to be resistant to
quantum computing. Lattice basis is just an image of what is already there (OKT). A low cost, pervasive and high entropy which hybridizes and increases AES capacity to roughly 10 times. Thereby, mitigates the effects of 
...
# Specification
To solve this problem, we used 3D shapes and their properties: Especially the face of a cuboid in Galois field. The properties like, face, edges and vertices come in handy in assumed infinite of finite space. You can get a flux from these properties as a result of vectors which forms regular points in Euclidean space to enhance orientation just like you see in lattice basis. To put all these terms programmatically (code) the idea of a matrix formation, transposition, transformation, substitution and modula Arithmetic serves us well. The face of a lattice will have points or vectors. Same goes to a matrix which is a representation of the lattice. [Therefore, Flux = double integral F*n dS where n=1] for the physicist drives the point home. However, entropy has the same effect for the mathematician and cryptographers. This means that any normal face in a shape will have a regular arrangement of point in Euclidean space (bais). In this sense, following the elements of Galois' field; a matrix, mathematically can hold a lattice's contents: It is then noted that a lattice ( reference favors the vertices) is only a blue print as well. It will have points upon which forces could interact. This means that changes in choosing any of these points could change the lattice or the matrix indices they bear. Below is the explanation of informational entropy.
...
It is also a standard practise to increase entropy by introducing a modulo arithmetic which carries (OR operation) to passphrases or characters and the original messages.
